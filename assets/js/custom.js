  ...
  if ($("#eff-embed-signup").length > 0) {
    if (document.cookie.indexOf('visited=true') === -1) {
      ...
      $(window).resize(function () {
        ...
        width = window.innerWidth < 600 ? "95%" : width;
        $.colorbox.resize({
          width: width // height: window.innerHeight > parseInt(cboxOptions.maxHeight) ? cboxOptions.maxHeight : cboxOptions.height
  ...
  *
  ...
  function init_slider() {
    ...
    var $slides = $container.css({
      //set container properties and get slides
      "position": "relative",
      "overflow": "hidden"
    }).find(".slide");

    if ($slides.length < 1) {
      //if no slides don't don anything else
      return;
    }

    $slides.eq(0).css({
      //set first slide to active and in view
      "position": "absolute",
      "width": "100%",
      "height": "100%",
      "top": 0,
    ...
    if ($slides.length < 2) {
      //if no more slides do nothing else, just show first slide
      return;
    }

    for (var i = 1; i < $slides.length; i++) {
      //for each slide after the first
      var $this = $slides.eq(i); //get slide as $this

      $this.css({
        //set css properties for slide
        "position": "absolute",
        "width": "100%",
        "height": "100%",
        "top": 0,
    ...
    function slide() {
      //recursive function to move slides
      timeout = setTimeout(function () {
        //set timeout
        var $active_slide = $slides.filter(".active").removeClass("active").animate({
          //animate the active slide out of view and remove class active
          "left": "-100%"
        }, 1000, function () {
          $(this).css({
            //move back to the right with all the other slides off screen
            "left": "100%"
          });
        ...
        $next_slide.addClass("active").animate({
          //animate slide into view and add class active
          "left": "0"
        }, 1000);
  ...
  $('.filter-term').click(function () {
    ...
    var filters = current_page_query ? current_page_query[2] === "" ? Array() : current_page_query[2].replace("%2C", ",").split(",") : Array();
